...
<div style="border:0px black solid;width:170px;padding:0px;display:inline-table;vertical-align:top;">
	...
	{checkActionsTpl location="tpl_pligg_story_start"}
		...
		<div class="headline">
			...
			{/if}			
				<div style="border:1px solid black;min-height:100px;margin-bottom:10px;">
				Image placeholder
				</div>
				<!-- 
				<div class="vote">
				...
				{if $use_title_as_link eq true}
				 ...
				 {else}
					{if $pagename eq "story" && $url_short neq "http://" && $url_short neq "://"}
						<a href="{$url}" {if $open_in_new_window eq true} target="_blank"{/if} {if $story_status neq "published"}rel="nofollow"{/if}>{$title_short}</a>
					{else} 
					  <a href="{$story_url}">{$title_short|truncate:20:".."}</a>
					{/if}
				{/if}
